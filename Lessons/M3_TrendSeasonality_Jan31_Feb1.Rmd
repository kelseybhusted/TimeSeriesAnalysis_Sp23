...
detrend <- residuals(linear_trend)
...
detrend <- (inflow_data[,(iHP+1)]) - (beta0+beta1*t)
...
plot(detrend, type = "l", main = "Detrended HP Graph", col = "green")
...
dummies <- seasonaldummy(ts_inflow_data[,iHP+1])

seas_means_models <- lm(inflow_data[,iHP+1] ~ dummies)

beta0 <- seas_means_models$coefficients[1]
beta1 <- seas_means_models$coefficients[2:12]
