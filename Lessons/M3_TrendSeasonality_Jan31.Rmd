...
```{r}
ts_inflow <- ts(inflow_data[,2:16], frequency = 12, start = c(1931,1)) 
head(inflow_data)
```
...
for(i in 1:nhydro){
  par(mfrow=c(1,2))
  Acf(ts_inflow[,i],lag.max=40,plot=TRUE, main=paste0("HP", i))
  Pacf(ts_inflow[,i],lag.max=40,plot=TRUE, main=paste0("HP", i))
...
```{r}
#Create vector t
t= 1:nrow(inflow_data)
iHP = 4

#dummies <- seasonaldummy(ts_inflow[,4])
