...
```{r Question 1}
#ts transformation
ts_energy_data <- ts(energy_data[,2], frequency = 12, start = c(1973,1)) 
 as.numeric(energy_data$Total.Renewable.Energy.Production)
diff_ts <- diff(ts_energy_data, lag = 1, differences = 1)

par(mfrow = c(1,2))
plot(ts_energy_data, types="l", col = "blue", ylab = "Renewable Energy Prod. TS", main = "Time Series")
plot(diff_ts, type = "l", col="blue", ylab = "Renewable Energy Prod. TS", main = "Differenced TS ")
...
energy_df <- data.frame(date ,RenewableEnergyProduction, Detrend, diff_ts )
...
```{r}
#Use ggplot
ggplot(energy_df, aes(x = date, y = RenewableEnergyProduction))+
  geom_line()
```
